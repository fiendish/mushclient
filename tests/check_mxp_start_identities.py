...
def main():
    ...
    declaration = between(header, '  bool MXP_OpenAtomicTag', '  void MXP_CloseAtomicTag')
    ...
    for name in packet_names:
        case = between(atomic, '    case MXP_ACTION_' + name + ':', '          break;') if name in ['VERSION', 'AFK', 'SUPPORT'] else atomic[atomic.index('    case MXP_ACTION_' + name + ':'):]
    ...
    pieces = {
        'TRANSACTION': between(start, 'class CMXPStartTransaction', 'class CActionReferenceGuard'),
        ...
        'HR_CASE': between(atomic, '    case MXP_ACTION_HR:', '    case MXP_ACTION_PRE:'),
        'LI_CASE': between(atomic, '     case MXP_ACTION_LI:', '    // pueblo tags'),
        'IMAGE_RESULT': between(atomic, '            CStyle * pResultStyle = AddStyle', '            strAction = strOldAction;'),
    }
