...
import subprocess
import tempfile
from output_callbacks import section
...
def run(source, output, revision):
    ...
    shim = shim.replace('#include <cassert>', '#include <cassert>\n#include <type_traits>')
    shim = shim.replace('template<class T> struct List', '''
...
template<class T> struct List''')
    shim = shim.replace('T GetNext(POSITION& p)const{', 'T GetNext(POSITION& p)const{countRead<T>();')
    shim = shim.replace('T GetPrev(POSITION& p){', 'T GetPrev(POSITION& p){countRead<T>();')
    shim = shim.replace('struct AppType {', 'struct AppType {long long GetUniqueNumber(){return ++seq;}')
    shim = shim.replace('POSITION m_pLinePositions[11]={};',
                        'POSITION positionStorage[10002]={};POSITION* m_pLinePositions=positionStorage;')
    shim = shim.replace(' long long m_iOutputGeneration=0;',
