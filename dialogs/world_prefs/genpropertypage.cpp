...
static void CopyPropertyRuntimeState (CObject * pOldItem, CObject * pNewItem,
  ...
  if (pOldItem->IsKindOf (RUNTIME_CLASS (CTrigger)))
    ...
    pNew->bSelected = pOld->bSelected;
    pNew->iUserOption = pOld->iUserOption;
    // Preserve destination names that the dialog does not edit.
    if (pNew->iSendTo != eSendToVariable)
      pNew->strVariable = pOld->strVariable;
#ifdef PANE
    if (pNew->iSendTo != eSendToPane)
      pNew->strPane = pOld->strPane;
#endif // PANE
...
void CGenPropertyPage::OnAddItem(CDialog & dlg) 
{
 ...
 CString strObjectName;
  ...
  // Script lookup can add an object with this label before we publish ours.
  CObject * pExistingItem;
  if (m_ObjectMap->Lookup (strObjectName, pExistingItem))
    {
    CString strMsg;
