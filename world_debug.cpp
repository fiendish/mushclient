...
static CString FindTimerName (CTimerMap & timerMap, const CTimer * pTimer)
  {
  for (POSITION pos = timerMap.GetStartPosition (); pos; )
    {
    CString strName;
    CTimer * pMappedTimer;
    timerMap.GetNextAssoc (pos, strName, pMappedTimer);
...
//-----------------------------------------------------------------------
    ...
    for (map<double,CTimer *>::const_iterator it = sortedTimers.begin ();
      ...
      if (pLabel [0] == 0)
        {
        strTimerName = FindTimerName (GetTimerMap (), pTimer);
        pLabel = strTimerName;
        }
